...
async def health_security(request: web.Request) -> web.Response:
    ...
    Принцип работы: Собирает информацию о сертификатах, токенах, анализирует заголовки текущего запроса,
                   проверяет подпись, сериализует локальный кэш blocked_users, а также возвращает
                   нормализованный список endpoint-ов, участвующих в блокировке пользователей
    Входящие параметры: request - объект HTTP запроса
    ...
    normalized_endpoint_userblocked = []
    for endpoint in getattr(config, 'endpoint_userblocked', []) or []:
        normalized_endpoint = str(endpoint).strip().strip('/').lower()
        if normalized_endpoint:
            normalized_endpoint_userblocked.append(normalized_endpoint)
    ...
    security_data = {
        ...
        "endpoint_userblocking": {
            "enabled": len(normalized_endpoint_userblocked) > 0,
            "count": len(normalized_endpoint_userblocked),
            "normalized_endpoints": normalized_endpoint_userblocked
        },
        "blocked_users": {
